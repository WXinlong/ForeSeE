...
from depth_normal_model_maxp_bfsoftmax import DepthNormal
...
# Add by users
#pcd_folder = os.path.join(BASE_DIR, 'output')
...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        save_bin(pcd_velo, rgb, os.path.join(pcd_folder, img_name) + '.bin', sample_mask=None)
        #save_ply(pcd_velo, rgb, os.path.join(pcd_folder, img_name) + '.ply', sample_mask=None)
...
def save_bin(pcd, rgb, path, sample_mask=None):
    width = rgb.shape[1]
    height = rgb.shape[0]
    xyz = np.reshape(pcd, [3, width * height])
    ...
    # remove points higher than 1m.
    height_filter = xyz[:, 2] <= 1. 
    xyz = xyz[height_filter, :]

    # add reflectance
    r = np.ones([xyz.shape[0], 1])
    xyzr = np.hstack([xyz, r]).astype(np.single)
