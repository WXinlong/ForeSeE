...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        pred_depth = np.squeeze(out['b_fake']) * 80.
        ...
        save_ply(pcd_velo, rgb, os.path.join(pcd_folder, img_name) + '.ply')
...
def save_ply(pcd, rgb, path):
    width = rgb.shape[1]
    height = rgb.shape[0]
    x = np.reshape(pcd[0], (width * height, 1))
    y = np.reshape(pcd[1], (width * height, 1))
    z = np.reshape(pcd[2], (width * height, 1))
    x = np.squeeze(x)
    y = np.squeeze(y)
    z = np.squeeze(z)
