...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        save_ply(pcd_velo, rgb, os.path.join(pcd_folder, img_name) + '_sample_diff.ply', sample_mask=sample_mask)
...
def get_sample_mask(conf, threshold):
    max_conf = np.max(conf, axis=0, keepdims=True) # [h, w]
    max_mask = conf == max_conf
    conf[max_mask] = 0.
    secmax_conf = np.max(conf, axis=0, keepdims=True)
    diff_conf = max_conf = secmax_conf
    diff_conf = np.squeeze(diff_conf)
    return diff_conf >= threshold
