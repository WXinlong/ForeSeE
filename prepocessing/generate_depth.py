...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate Depth')
    ...
    lidepth_dir = args.data_path + '/lidepth/'

    assert os.path.isdir(lidar_dir)
    assert os.path.isdir(calib_dir)
    assert os.path.isdir(image_dir)

    if not os.path.isdir(lidepth_dir):
        os.makedirs(lidepth_dir)
    ...
    for fn in lidar_files:
        ...
        lidepth = generate_depth_from_velo(lidar, height, width, calib)
        np.save(lidepth_dir + '/' + predix, lidepth)
