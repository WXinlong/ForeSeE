...
from depth_normal_model import *
...
def train(train_dataloader, model, epoch, loss_func,
    ...
    for i, data in enumerate(train_dataloader):
        ...
        out = model(data)
        losses = loss_func.criterion(out['b_fake'], out['b_fake_nosoftmax'], out['b_fake_roi'], out['b_fake_roi_nosoftmax'],  data, epoch)
...
def val_kitti(val_dataloader, model):
    ...
    rois_smoothed_criteria = {'err_absRel': rois_smoothed_absRel, 'err_silog': rois_smoothed_silog, 'err_silog2': rois_smoothed_silog2}
    ...
    bg_smoothed_criteria = {'err_absRel': bg_smoothed_absRel, 'err_silog': bg_smoothed_silog, 'err_silog2': bg_smoothed_silog2}
    for i, data in enumerate(val_dataloader):
        ...
        rois_smoothed_criteria = validate_err_kitti(pred_depth['b_fake'], data['B_raw_rois'], rois_smoothed_criteria)
        bg_smoothed_criteria = validate_err_kitti(pred_depth['b_fake'], data['B_raw_bg'], bg_smoothed_criteria)
    ...
    rois_val_metrics = {'abs_rel': rois_smoothed_criteria['err_absRel'].GetGlobalAverageValue(),
            'silog': np.sqrt(rois_smoothed_criteria['err_silog2'].GetGlobalAverageValue() - (rois_smoothed_criteria['err_silog'].GetGlobalAverageValue())**2)}
    bg_val_metrics = {'abs_rel': bg_smoothed_criteria['err_absRel'].GetGlobalAverageValue(),
            'silog': np.sqrt(bg_smoothed_criteria['err_silog2'].GetGlobalAverageValue() - (bg_smoothed_criteria['err_silog'].GetGlobalAverageValue())**2)}
