...
class fcn_topdown(nn.Module):
    def __init__(self, conv_body_func):
        ...
        self.top = nn.Sequential(
            nn.Conv2d(self.dim_in[0] * 5, self.dim_in[0], 1, stride=1, padding=0, bias=False),
        ...
        self.topdown_fcn1_roi = fcn_topdown_block(self.dim_in[0], self.dim_out[0])
        self.topdown_fcn2_roi = fcn_topdown_block(self.dim_in[1], self.dim_out[1])
        self.topdown_fcn3_roi = fcn_topdown_block(self.dim_in[2], self.dim_out[2])
        self.topdown_fcn4_roi = fcn_topdown_block(self.dim_in[3], self.dim_out[3])
        self.topdown_fcn5_roi = fcn_last_block(self.dim_in[4], self.dim_out[4])
        self.topdown_predict_roi = fcn_topdown_predict(self.dim_in[5], self.dim_out[5])
    ...
    def _fuse_roi(self, x_g, x_l, boxes):

        for idx in range(x_g.size(0)):
            cur_boxes = boxes[idx]
            #cur_mask = torch.FloatTensor(x_l.size()).zero_().to(device=x_l.device)
            for idx_box in range(cur_boxes.size(0)):
                box = cur_boxes[idx_box]
                if box.sum() == 0:
                    continue
                x1, y1, x2, y2 = map(int, box)
                #cur_mask[idx, :, y1:y2, x1:x2] = 1.
                x_g[idx, :, y1:y2, x1:x2] = x_l[idx, :, y1:y2, x1:x2] 
            #x_g += cur_mask * x_l
        return x_g

    def forward(self, laterals, backbone_stage_size, boxes):
        ...
        x1_roi = self.topdown_fcn1_roi(laterals[1], x)
        x2_roi = self.topdown_fcn2_roi(laterals[2], x1_roi)
        x3_roi = self.topdown_fcn3_roi(laterals[3], x2_roi)
        x4_roi = self.topdown_fcn4_roi(laterals[4], x3_roi)
        x5_roi = self.topdown_fcn5_roi(x4_roi, backbone_stage_size)
        x6_roi = self.topdown_predict_roi(x5_roi) 

        #x6_final = self._fuse_roi(x6[0], x6_roi[0], boxes)
